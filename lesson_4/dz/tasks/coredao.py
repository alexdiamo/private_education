...
class CoredaoBridge(Base):
    async def bridge_usdt_bsc_to_usdt_coredao(self, amount: Optional[TokenAmount] = None, max_fee: float = 1):
        ...
        if usdt_balance.Wei < amount.Wei:
            return f'{failed_text}: {self.client.account.address} | not enough balance ({usdt_balance.Ether} / {amount.Ether})'
        ...
        if bsc_balance.Wei < fee.Wei:
            return f'{failed_text}: {self.client.account.address} not enought native token | balance: {bsc_balance.Ether} | fee: {fee.Ether}'
